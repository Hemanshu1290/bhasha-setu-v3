...
# ---------- Input validation (used at write-time, not just extraction-time) ----------
def valid_mobile(m):
    """10 digits, no letters — matches the same rule the extractors already promise."""
    return bool(re.fullmatch(r'\d{10}', str(m or '').strip()))

def valid_name(n):
    n=str(n or '').strip()
    return 1 < len(n) <= 60

def mobile_exists(mobile, exclude_citizen_id=None):
    for p in rows('Citizens'):
        if str(p.get('Mobile',''))==str(mobile) and p.get('Citizen_ID')!=exclude_citizen_id:
            return True
    return False
...
def extract_regex(text):
    ...
    if any(x in t for x in ['apply','application','आवेदन','apply karna']):
        ...
        intent='APPLICATION_STATUS'
                               ...
                               'tell me','tell','about','what is','explain','details','documents','know about','eligib']):
        intent='SCHEME_INFORMATION'
    elif any(x in t for x in ['search','find','खोज']):
        intent='SEARCH_CITIZEN'
    elif scheme:
        # A specific scheme was named but no action verb was found — treat as an info request
        # rather than falling through to a dead-end generic reply.
        intent='SCHEME_INFORMATION'

    name=None; city=None; address=None; mobile=None

    m=re.search(r'(?:naam|name)\s+(?:is\s+|hai\s+)?([A-Za-z][A-Za-z ]{1,40}?)(?:,|\s+hai|\s+is\s|\s+main|\s+aur|$)',text,re.I)
...
def confirm(req:ConfirmRequest):
    ...
    if 'mobile' in req.updates and not valid_mobile(req.updates['mobile']):
        raise HTTPException(400,'Mobile number must be exactly 10 digits.')
    if 'mobile' in req.updates and mobile_exists(req.updates['mobile'], exclude_citizen_id=req.citizen_id):
        raise HTTPException(409,'Another citizen already has this mobile number.')
    if 'name' in req.updates and not valid_name(req.updates['name']):
        raise HTTPException(400,'Name must be between 2 and 60 characters.')
...
def confirm_insert(req:ConfirmInsertRequest):
    ...
    if not valid_name(nc.get('name')):
        raise HTTPException(400,'Name must be between 2 and 60 characters.')
    if not valid_mobile(nc.get('mobile')):
        raise HTTPException(400,'Mobile number must be exactly 10 digits.')
    if mobile_exists(nc.get('mobile')):
        raise HTTPException(409,'A citizen with this mobile number already exists.')
    if nc.get('scheme') and nc['scheme'] not in SCHEMES_INFO:
        raise HTTPException(400,f'Unknown scheme: {nc["scheme"]}')
...
def confirm_apply(req:ConfirmApplyRequest):
    ...
    if req.scheme not in SCHEMES_INFO:
